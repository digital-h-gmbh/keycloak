...
@NamedQueries({
        ...
        @NamedQuery(name="getGroupIdsByNameContainingFromIdList", query="select u.id from GroupEntity u where u.realm = :realm and u.name like concat('%',:search,'%') and u.id in :ids order by u.name ASC"),
        ...
        @NamedQuery(name="getGroupCountByNameContainingFromIdList", query="select count(u) from GroupEntity u where u.realm = :realm and u.name like concat('%',:search,'%') and u.id in :ids"),
